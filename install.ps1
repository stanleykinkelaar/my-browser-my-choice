...
$scriptDest = Join-Path $appDir "set-default-browser.ps1"
...
# Remove older install locations from previous versions
...
$link.Arguments  = "-NoProfile -File `"$scriptDest`""
$link.Save()
...
& powershell.exe -NoProfile -File $scriptDest
